...
namespace ChallengeTest
{
    public class ChallengeTest
    {
        ...
        public void PermissionSet()
        {
            ...
            PermissionSet permissionSet = new PermissionSet(new List<int> { perm1, perm66 });
            ...
            Assert.False(permissionSet.Permissions.Any());

            Assert.Throws<Exception>(() => new PermissionSet(new List<int> { perm0, perm101 }));
            Assert.Throws<Exception>(() => new PermissionSet(new List<int> { perm1, perm101 }));
            Assert.Throws<Exception>(() => new PermissionSet(new List<int> { perm1, perm0 }));

            byte[] bytes = new List<int> { perm1, perm66 }.SelectMany(BitConverter.GetBytes).ToArray();
            Assert.Contains(perm1, new PermissionSet(bytes).Permissions);
            Assert.Contains(perm1, new PermissionSet(bytes).Permissions);

            Assert.Throws<Exception>(() => new PermissionSet(new List<int> { perm0 }.SelectMany(BitConverter.GetBytes).ToArray()));
            Assert.Throws<Exception>(() => new PermissionSet(new List<int> { perm101 }.SelectMany(BitConverter.GetBytes).ToArray()));
        }

        [Fact]
        public void UnsetUser()
        {
            BlobIO blob = new User().ToBlobIO();

            Assert.Equal(DateTime.MinValue, blob.GetDateTime(0));
            Assert.Null(blob.GetString(1));
            Assert.Null(blob.GetPermissionSet(2));
            Assert.Null(blob.GetString(3));
            Assert.Null(blob.GetString(4));
        }
        ...
        public void UserToBlob()
        {
            ...
            int perm66 = 66;
            int perm101 = 101;
            PermissionSet ps = new PermissionSet(new List<int> { perm1, perm66 });
            string tz = "PST";
            string name = "Name";
